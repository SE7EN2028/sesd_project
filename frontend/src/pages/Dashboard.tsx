...
interface Overview {
    ...
    recentActivity: { event: string; data: any; time: string }[];
}
...
export default function Dashboard() {
    ...
    const s = data.todos;
    const cards = [
        { label: 'Total Tasks', value: s.total, cls: 'g1', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2' },
        { label: 'Completed', value: s.completed, cls: 'g3', icon: 'M20 6L9 17l-5-5' },
        { label: 'In Progress', value: s.pending, cls: 'g2', icon: 'M12 2a10 10 0 100 20 10 10 0 000-20zM12 6v6l4 2' },
        { label: 'Overdue', value: s.overdue, cls: 'g4', icon: 'M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z' },
    ];

    const types = [{ key: 'personal', color: '#7c5cfc' }, { key: 'work', color: '#3a86ff' }, { key: 'urgent', color: '#ff6b6b' }];
                                ...
                                <div key={t.key} className="bd-row">
                                    ...
                                    <div className="bd-bar-wrap">
                                        <motion.div className="bd-bar" style={{ background: t.color }}
                                            initial={{ width: 0 }}
                                            animate={{ width: `${s.total ? ((s.byType[t.key] || 0) / s.total) * 100 : 0}%` }}
                                            transition={{ duration: 0.8, delay: 0.4 + i * 0.1 }} />
                                    </div>
                                    <span className="bd-val">{s.byType[t.key] || 0}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="card">
                            ...
                            <div className="activity-feed">
                                {data.recentActivity.map((a, i) => (
                                    <motion.div key={i} className="activity-row" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.04 }}>
                                        <div className={`activity-dot ${a.event.includes('deleted') ? 'dot-danger' : a.event.includes('completed') ? 'dot-success' : 'dot-primary'}`} />
                                        <div className="activity-body">
                                            <span className="activity-action">{formatEvent(a.event)}</span>
                                            {a.data?.title && <span className="activity-detail">{a.data.title}</span>}
                                        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div className="card">
                        ...
                        <div className="progress-section">
                            <Ring value={s.completed} max={s.total} />
                            <div className="ring-info">
                                <div className="ri-big"><AnimatedNumber value={s.completed} /> of {s.total}</div>
                                <div className="ri-small">tasks completed</div>
                            </div>
                        </div>
                        <div className="prio-list">
                            {[{ key: 'high', color: 'var(--danger)' }, { key: 'medium', color: 'var(--warning)' }, { key: 'low', color: 'var(--accent)' }].map(p => (
                                <div key={p.key} className="prio-row">
                                    <span className="prio-label"><span className="prio-dot" style={{ background: p.color }} />{p.key}</span>
                                    <span className="prio-val" style={{ color: p.color }}>{s.byPriority[p.key] || 0}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="card">
                        ...
                        <div className="quick-stats">
                            <div className="qs-row"><span className="qs-label">Due Today</span><span className="qs-val" style={{ color: 'var(--warning)' }}>{s.dueToday}</span></div>
                            <div className="qs-row"><span className="qs-label">Overdue</span><span className="qs-val" style={{ color: 'var(--danger)' }}>{s.overdue}</span></div>
                            <div className="qs-row"><span className="qs-label">Categories</span><span className="qs-val">{data.totalCategories}</span></div>
                        </div>
                    </div>
                </div>
            </motion.div>
